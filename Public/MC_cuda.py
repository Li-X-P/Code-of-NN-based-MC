...
class MLP(torch.nn.Module):
    ...
    def __init__(self, input_shape, rank):
        ...
        self.FC = torch.nn.Sequential(
            ...
            torch.nn.ELU(),
            # torch.nn.Linear(rank+50, rank+20, bias = 1),
            # torch.nn.ELU(),
            # torch.nn.Linear(rank+20, rank+10),
            # torch.nn.ReLU(),
    ...
    def fit(self, TensorData, Omega = None, rank = None, epochs = 1000, lr = 0.01, batch_size = 512, mode='batch'):
        ...
        if self.tensorn_name == 'CP':
            ...
            for i in range(self.tensor_num):
                self.mode_nn.append(MLP(self.tensor_shape[i], rank).apply(init_weights).cuda())  # Neural Networks
...
if __name__ == "__main__":
    ...
    uv = model.fit(M_Omega, Omega = Omega, rank = 10, epochs = 2000, lr = 0.01, batch_size = 128, mode='full')
    toc = time.time()
    ...
    loss = lossfn(Tensor_result, M.cuda())
    
    print(f"Final MSE is {loss}")
    print(toc-tic)
    psnr = skm.peak_signal_noise_ratio(M.cpu().detach().numpy(),Tensor_result.cpu().detach().numpy())
    print(f"Final MSE is {loss}, PSNR is {psnr}")
    
    #np.save("Windows_Fixed005.npy",Tensor_result.detach().numpy())
    plt.figure()
    plt.imshow(Tensor_result.cpu().detach().numpy(),cmap='gray')
    plt.show()